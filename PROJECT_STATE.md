...
| 微信小程序 | `miniprogram/` | 首页联想与历史、配方/问答独立查询、Canvas 合成树、知识问答证据与干员档案；视觉已同步当前 Web 的官网素材、黄黑工业档案主视觉，首页不使用开场幕布；问答答案经轻量 markdown 渲染（`utils/markdown.js` → rich-text，避免 `**`、`*` 等原始符号暴露）；静态检查已通过，待开发者工具和真机验收 |
...
- **小程序视觉同步（2026-09）**：复用 Web 已归档的佩丽卡、管理员与地景官网素材，首页、查询结果、合成树和干员页统一为 `#FFFA00 / #282828`；移除首页旧开场幕布与 Q 版装饰，保留触控查询、历史、联想、媒体和 Canvas 交互。
